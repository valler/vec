...
export const basis: m = [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
export const set = (a: n, b: n, c: n): v => [a, b, c];
...
export const div = ([a, b, c]: v, [d, e, f]: v): v => [a / d, b / e, c / f];
export const pow = (
  [a, b, c]: v,
  [d, e, f]: v,
): v => [nPow(a, d), nPow(b, e), nPow(c, f)];
export const mad = (
  [a, b, c]: v,
  [d, e, f]: v,
  [g, h, i]: v,
): v => [a * d + g, b * e + h, c * f + i];
export const sAdd = ([a, b, c]: v, s: n): v => [a + s, b + s, c + s];
export const sSub = ([a, b, c]: v, s: n): v => [a - s, b - s, c - s];
export const sMul = ([a, b, c]: v, s: n): v => [a * s, b * s, c * s];
export const sDiv = ([a, b, c]: v, s: n): v => [a / s, b / s, c / s];
export const sPow = ([a, b, c]: v, s: n): v => [
  nPow(a, s),
  nPow(b, s),
  nPow(c, s),
];
...
export const dir = ([a, b, c]: v): v => {
  const s = sqrt(a * a + b * b + c * c);
  return [a / s, b / s, c / s];
};
export const dist = ([a, b, c]: v, [d, e, f]: v) => {
  const g = a - d;
  const h = b - e;
  const i = c - f;
  return sqrt(g * g + h * h + i * i);
};
export const sqrDist = ([a, b, c]: v, [d, e, f]: v) => {
  const g = a - d;
  const h = b - e;
  const i = c - f;
  return g * g + h * h + i * i;
};
export const area = ([a, b, c]: v, [d, e, f]: v): n => {
  const g = a * e - b * d;
  const h = a * f - c * d;
  const i = b * f - c * e;
  return sqrt(g * g + h * h + i * i);
...
export const coefSum = ([a, b, c]: v) => a + b + c;
export const coefPro = ([a, b, c]: v) => a * b * c;
export const proj = ([a, b, c]: v, [d, e, f]: v): v => {
  const s = a * d + b * e + c * f;
  return [s * a, s * b, s * c];
};
export const rej = ([a, b, c]: v, [d, e, f]: v): v => {
  const s = -a * d - b * e - c * f;
  return [s * a + d, s * b + e, s * c + f];
};
export const refl = ([a, b, c]: v, [d, e, f]: v): v => {
  const s = -2 * (a * d + b * e + c * f);
  return [s * a + d, s * b + e, s * c + f];
};
export const conj = ([a, b, c]: v, [d, e, f]: v): v => {
  const s = -a * d - b * e - c * f;
  return [
    -2 * (s * a + d) + d,
    -2 * (s * b + f) + e,
...
export const rot = ([a, b, c]: v, [d, e, f]: v, [g, h, i]: v): v => {
  let s = -2 * (a * g + b * h + c * i);
  const j = s * a + g;
  const k = s * b + h;
  const l = s * c + i;
...
export const lerp = ([a, b, c]: v, [d, e, f]: v, t: n): v => {
  const x = 1 - t;
  return [
    x * a + t * d,
    x * b + t * e,
