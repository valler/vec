...
const updateVertices = (gl, buf, vertices) => {
    const gla = gl.ARRAY_BUFFER;
    gl.bindBuffer(gla, buf);
    gl.bufferData(gla, new Float32Array(vertices), gl.DYNAMIC_DRAW);
    gl.bindBuffer(gla, null);
};
export const draw = async (ca, vertexShaderSource, fragmentShaderSource, geometry, options) => {
    ...
    enableAttribs(gl, pro, vao, vBuf, attribs, stride, gl.FLOAT, Float32Array.BYTES_PER_ELEMENT);
